...
export async function POST(_req: Request, { params }: any) {
  try {
    ...
    await prisma.careEvent.create({
      data: { ownerId: userId, plantId: updated.id, type: "water", waterMl: updated.waterMl ?? null },
    })
    // Auto-complete the nearest uncompleted water task scheduled up to end of today
    const now = new Date()
    ...
    const msg = e instanceof Error ? e.message : "Failed to water"
